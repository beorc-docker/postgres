...
map_postgres_uid() {
  USERMAP_ORIG_UID=$(id -u ${DB_USER})
  USERMAP_ORIG_GID=$(id -g ${DB_USER})
  USERMAP_GID=${USERMAP_GID:-${USERMAP_UID:-$USERMAP_ORIG_GID}}
  USERMAP_UID=${USERMAP_UID:-$USERMAP_ORIG_UID}
  if [[ ${USERMAP_UID} != ${USERMAP_ORIG_UID} ]] || [[ ${USERMAP_GID} != ${USERMAP_ORIG_GID} ]]; then
    echo "Adapting uid and gid for ${DB_USER}:${DB_USER} to $USERMAP_UID:$USERMAP_GID"
    groupmod -g ${USERMAP_GID} ${DB_USER}
...
create_data_dir() {
  ...
  chown -R ${DB_USER}:${DB_USER} ${PG_HOME}
}

create_log_dir() {
  mkdir -p ${PG_LOGDIR}
  chmod -R 1775 ${PG_LOGDIR}
  chown -R root:${DB_USER} ${PG_LOGDIR}
}
...
create_run_dir() {
  ...
  chmod g+s ${PG_RUNDIR}
  chown -R ${DB_USER}:${DB_USER} ${PG_RUNDIR}
}
...
if [[ ! -d ${PG_DATADIR} ]]; then
  if [[ ${PSQL_MODE} == slave || ${PSQL_MODE} == snapshot ]]; then
    ...
    if [[ ${PSQL_MODE} == snapshot ]]; then
      sudo -Hu ${DB_USER} \
        PGPASSWORD=$REPLICATION_PASSWORD ${PG_BINDIR}/pg_basebackup -D ${PG_DATADIR} \
...
hot_standby = on
EOF
      sudo -Hu ${DB_USER} touch ${PG_DATADIR}/recovery.conf
      cat >> ${PG_DATADIR}/recovery.conf <<EOF
standby_mode = 'on'
primary_conninfo = 'host=${REPLICATION_HOST} port=${REPLICATION_PORT} user=${REPLICATION_USER} password=${REPLICATION_PASSWORD} sslmode=${PSQL_SSLMODE}'
...
if [[ -n ${PG_OLD_VERSION} ]]; then
  ...
  sudo -Hu ${DB_USER} ${PG_BINDIR}/pg_upgrade \
...
sudo -Hu ${DB_USER} ${PG_BINDIR}/pg_ctl -D ${PG_DATADIR} -o "-c listen_addresses=''" -w start
...
sudo -Hu ${DB_USER} ${PG_BINDIR}/pg_ctl -D ${PG_DATADIR} -m fast -w stop
...
exec start-stop-daemon --start --chuid ${DB_USER}:${DB_USER} --exec ${PG_BINDIR}/postgres -- \
